...
#import "YGReaderUtil.h"

#import "UIView+XDSHyperLink.h"

@interface YGReadView () <DTAttributedTextContentViewDelegate>
{
...
- (instancetype)initWithFrame:(CGRect)frame chapterNum:(NSInteger)chapterNum pageNum:(NSInteger)pageNum {
    if (self = [super initWithFrame:frame]) {
        ...
        YGChapterModel *chapterModel = CURRENT_BOOK_MODEL.chapters[self.chapterNum];
...
-(void)showMagnifier{
    if (!_magnifierView) {
        self.magnifierView = [[YGMagnifierView alloc] init];
...
- (void)linkPushed:(DTLinkButton *)button{
    YGNoteModel *noteModel = [YGNoteModel getNoteFromURL:button.URL];
    if (noteModel) {
        [YGReaderUtil showAlertWithTitle:@"笔记内容" message:noteModel.content];
    }else{
        ...
        YGCatalogueModel *catalogueModel;
        for (YGChapterModel *chapterModel in CURRENT_BOOK_MODEL.chapters) {
            if ([url hasSuffix:chapterModel.chapterSrc]) {
                catalogueModel = [[YGCatalogueModel alloc] init];
        ...
        if (catalogueModel) {
            ...
            YGChapterModel *chapterModel = CURRENT_BOOK_MODEL.chapters[selectedChapterNum];
...
-(void)menuCopy:(id)sender{
    ...
    [YGReaderUtil showAlertWithTitle:@"成功复制以下内容" message:pasteboard.string];
...
- (void)addLineForNote:(YGNoteModel *)noteModel{
